...
static bool setup_stack(void **esp, struct arguments *pArguments);
...
void * push_arguments(struct arguments *args, void **esp) {
    ...
    *esp -= sizeof(uint32_t);
    ...
    for (int i = args->argc - 1; i >= 0; i--) {
        void * arg_i_addr = arg_ptr_list[i];
        *esp -= sizeof(void *);
        memcpy(*esp, arg_i_addr, sizeof(void *));

    }
    // push argv
    *esp -= sizeof(void *);
    memcpy(*esp, esp, sizeof(void *));
    // push argc
    *esp -= sizeof(int);
